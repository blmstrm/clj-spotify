...
(deftest test-get-an-album
  (testing "Get a spotify album and verify the json data to be equal to test data in album.json"
    ( with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/album-data-file))
            differences (data/diff (sptfy/get-an-album {:id "0sNOF9WDwhWunNAHPD3Baj"} util/spotify-oauth-token) correct-test-data)]  
...
(deftest test-get-several-albums
  (testing "Get several spotify albums and verify the json data to be equal to test data in albums.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/albums-data-file))
            differences (data/diff (sptfy/get-several-albums {:ids ["41MnTivkwTO3UUJ8DrqEJJ" ,"6JWc4iAiJ9FjyK0B59ABb4","6UXCm6bOO4gFlDQZV5yL37"]} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-tracks-of-an-album
  (testing "Get the tracks of a spotify album and verify the json data to be equal to test data in tracks-of-an-album.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/track-of-album-data-file))
            differences (data/diff (sptfy/get-an-albums-tracks {:id "6akEvsycLGftJxYudPjmqK"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))

(deftest test-get-an-artist
  (testing "Get an artist and verify the json data to be equal to test data in artist.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/artist-data-file))
            differences (data/diff  (sptfy/get-an-artist {:id "0OdUWJ0sBjDrqHygGUXeCF"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-several-artists
  (testing "Get several artists and verify the json data to be equal to test data in artist.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/artists-data-file))
            differences (data/diff ( sptfy/get-several-artists {:ids ["0oSGxfWSnnOXhD2fKuz2Gy","3dBVyJ7JuOMt4GE9607Qin"]} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-an-artists-albums
  (testing "Get an artists albums and verify the json data to be equal to test data in artist.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/artists-albums-file))
            differences (data/diff (sptfy/get-an-artists-albums {:id "1H1jG5SxsBzeuUk3ktdbeG"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-a-list-of-browse-categories
  (testing "Get a list of spotify browse categories and verify the json data to be equal to test data in list-of-browse-categories.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/browse-categories-file))
            differences (data/diff ( sptfy/get-a-list-of-categories {:locale "sv_SE" :country "SE" :limit 10 :offset 5}  util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-a-category
  (testing "Get a spotify category and verify the json data to be equal to test data in category.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/category-file))
            differences (data/diff ( sptfy/get-a-category {:category_id "dinner" :locale "sv_SE" :country "SE"}  util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-a-playlist
  (testing "Get a spotify playlist and verify the json data to be equal to test data in user-playlist.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/playlist-file))
            differences (data/diff (sptfy/get-a-playlist {:owner_id "elkalel" :playlist_id "5X6O7Wb8y3we9VzYTT9l64" :market "SE"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-a-playlists-tracks
  (testing "Get a spotify playlist's tracks and verify the json data to be equal to test data in playlists-tracks.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/playlists-tracks-file))
            differences (data/diff (sptfy/get-a-playlists-tracks {:owner_id "elkalel" :playlist_id "5X6O7Wb8y3we9VzYTT9l64" :offset 0 :limit 100 :market "SE"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
...
(deftest test-get-a-users-profile
  (testing "Get a users profile and verify the json data to be equal to test data in user-profile.json"
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/user-profile-file))
            differences (data/diff (sptfy/get-a-users-profile {:user_id "elkalel"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))

(deftest test-search
  (testing "Search spotify."
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/search-file))
            differences (data/diff (sptfy/search {:q "Muse" :type "track,artist" :market "US" :limit 1 :offset 5} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))

(deftest test-get-a-track
  (testing "Get information about a single track."
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/get-a-track-file))
            differences (data/diff (sptfy/get-a-track {:id "1zHlj4dQ8ZAtrayhuDDmkY" :market "ES"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))

(deftest test-get-several-tracks
  (testing "Get information about several tracks."
    (with-redefs [sptfy/json-string-to-map util/test-json-string-to-map]
      (let [correct-test-data (util/test-json-string-to-map (slurp tf/get-several-tracks-file))
            differences (data/diff (sptfy/get-several-tracks {:ids "7ouMYWpwJ422jRcDASZB7P,4VqPOruhp5EdPBeR92t6lQ,2takcwOaAZWiXQijPHIx7B" :market "ES"} util/spotify-oauth-token) correct-test-data)]
        (is (= nil (first differences) (second differences)))))))
